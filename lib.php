...
function annotate_add_instance(stdClass $annotate, mod_annotate_mod_form $mform = null) {
    ...
    $new_id = $DB->insert_record('annotate', $annotate, true);
    $annotate->id = $new_id;
    annotate_grade_item_update($annotate);
    return $new_id;
...
function annotate_grade_item_update(stdClass $annotate) {
    ...
    /** @example */
    $params = array();
    $params['itemname'] = clean_param($annotate->name, PARAM_NOTAGS);
    
    
    if ($annotate->grade > 0) {
        $params['gradetype'] = GRADE_TYPE_VALUE;
        $params['grademax']  = $annotate->grade;
        $params['grademin']  = 0;

    } else if ($annotate->grade < 0) {
        $params['gradetype'] = GRADE_TYPE_SCALE;
        $params['scaleid']   = -$annotate->grade;

    } else {
        $params['gradetype'] = GRADE_TYPE_TEXT; // allow text comments only
    }

    grade_update('mod/annotate', $annotate->course, 'mod', 'annotate', $annotate->id, 0, null, $params);
}
